"""
Usage:
  replay_edits.py [-v] [-f <outdir>] <datazip> [<student_id>]
...
if __name__ == '__main__':
    ...
    for s in dataset.students():
        if student_id and student_id != s.id():
            continue
        for wh in dataset.work_histories_for_student(s):
            print("Student {}, problem {}".format(wh.student_id(), wh.assign_num()))
            ...
            for evt in wh.events():
                if type(evt) is progsnap.Edit:
                    ...
                    if verbose:
                        print("{} {} {} -------------------------".format(wh.student_id(),evt.editid(),evt.type()))
